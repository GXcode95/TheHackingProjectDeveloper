...
const reloadArticles = (keyword) => {
    ...
    //trie data.articles et renvoie moi les articles dont le title comprend keywords
    let articlesToShow = data.articles.filter(article => article.title.includes(keyword))
     data.articles.forEach(article =>{
        if ( article.tags.filter(tag => tag == keyword).length && articlesToShow.includes(article) == false ) {
